...
@main_bp.route('/semantic-graph')
@login_required
def semantic_graph():
    """User profile page with semantic graph for premium users"""
    # Get the currently logged-in user's registered profile
    user = current_user
    registered_user = RegisteredUser.query.filter_by(user_id=user.user_id).first()
    
    if not registered_user:
        flash('Your profile is incomplete. Please update your information.', 'warning')
        return redirect(url_for('main.dashboard'))
...
def edit_topics():
    ...
    return redirect(url_for('main.semantic_graph'))
