...
def inbox():
    ...
    try:
        ...
        user_ids = db.session.query(Message.sender_id)\
            .filter(Message.recipient_id == current_user.user_id)\
...
def delete_message(message_id):
    ...
    if message.sender_id == current_user.user_id:
        return redirect(url_for('messaging.messages', user_id=message.recipient_id))
    else:
...
def unread_message_count():
    ...
    try:
        ...
        try:
            count = Message.query.filter_by(recipient_id=current_user.user_id, is_read=False).count()
        except Exception as e:
            ...
            cursor = conn.cursor()
            cursor.execute('SELECT COUNT(*) FROM messages WHERE recipient_id = ? AND is_read = 0', 
